...
use crate::ir;
use crate::ir::lower::{Lower, LowerContext};
use crate::lex::Span;
...
impl Compiler {
    ...
    pub fn compile(&mut self, import: SchemaImport) {
        ...
        let errors = super::register::TypeCollector::register(
            &ast,
            &import,
            &mut self.symbols,
            &self.import_roots,
    ...
    fn parse(&mut self, import: &SchemaImport) -> Result<ast::Schema, Vec<Diagnostic>> {
        ...
        // Lex
        let (tokens, lex_errors) = crate::lex::lex(&contents, import.clone());
    ...
    // FIXME: Make this declarative and set during `Compiler` construction.
    /// Runs all semantic analyzers on the AST.
    fn run_analyzers(&mut self, ast: &ast::Schema) {
        // Declarative list of analyzers to run
        self.diagnostics
            .extend(Self::run_analyzer(ast, FieldIndexUniqueness::new()));
        self.diagnostics.extend(Self::run_analyzer(
            ast,
            TypeReferenceResolver::new(&self.symbols),
        ));

        // FIXME: Add more analyzers (encoding validation, etc.)
    }
    ...
    fn lower_and_merge(&mut self, ast: &ast::Schema) {
        let root = Descriptor {
            package: PackageName::try_from(vec!["root".to_string()]).unwrap(),
            path: vec![],
            name: None,
        };
        let ctx = LowerContext::new(&self.symbols, root);
        ...
        // Merge into existing package or add new
    ...
    /// Consumes the compiler and returns the accumulated IR.
    pub fn into_ir(self) -> ir::Schema {
        self.ir
    }
}
