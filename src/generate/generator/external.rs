...
impl ExternalGenerator {
    ...
    pub fn new(binary_path: impl Into<PathBuf>) -> Result<Self, GeneratorError> {
        ...
        if !binary_path.is_executable() {
            return Err(GeneratorError::Process(format!(
                "generator binary not found/executable: {}",
                binary_path.display()
            )));
        }
    ...
    fn generate(&self, schema: &ir::Schema) -> Result<GeneratorOutput, GeneratorError> {
        ...
        if let Some(mut stdin) = child.stdin.take() {
            stdin
                ...
                .map_err(|e| GeneratorError::Process(format!("failed to write to stdin: {}", e)))?;
        }

        // Wait for process to complete
        let output = child
            .wait_with_output()
            .map_err(|e| GeneratorError::Process(format!("failed to wait for process: {}", e)))?;
