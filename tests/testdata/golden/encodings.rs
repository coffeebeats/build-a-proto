...
impl Encoded {
    ...
    fn encode_into(&self, w: &mut _baproto::BitWriter) -> Result<(), _baproto::EncodeError> {
        w.write_bits(self.compact as u64, 16)?;
        w.write_bits(_baproto::zigzag_encode(self.signed_compact as i64) as u64, 32)?;
        let variable_bits = if self.variable == 0 { 1 } else { 64 - self.variable.leading_zeros() } as u8;
        w.write_bits(variable_bits as u64, 8)?;
        w.write_bits(self.variable as u64, variable_bits)?;
        w.write_bits(self.delta_encoded as u64, 32)?;
        w.write_bits(_baproto::fixed_point_encode(self.fixed_val as f64, 16).to_bits() as u64, 32)?;
        w.write_bits(self.padded as u64, 32)?;
        w.pad(8)?;
        Ok(())
    }
    ...
    fn decode_from(r: &mut _baproto::BitReader) -> Result<Self, _baproto::DecodeError> {
        let compact = r.read_bits(16)? as u32;
        let signed_compact = _baproto::zigzag_decode(r.read_bits(32)? as i32);
        let _var_bits = r.read_bits(8)? as u8;
        let variable = r.read_bits(_var_bits)? as u64;
        let delta_encoded = r.read_bits(32)? as i32;
        let fixed_val = _baproto::fixed_point_decode(f32::from_bits(r.read_bits(32)? as u32), 16);
        let padded = r.read_bits(32)? as u32;
        r.skip(8)?;
        Ok(Self {
            compact,
            signed_compact,
